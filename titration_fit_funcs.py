...
def main():
    ...
    for df in dfs:
        ...
        guess=[[8.2,1.3,0.006],[8.4,1.1,0.0055],[8.7,0.8,0.006],\
               [8.7,0.9,0.006],[10,0.62,0.007]]
        fit_sym = fit_hill_symfit(df,guess[count],Va=Vas[count],equiv=equiv_vol)
        sim_mono_hill_equiv(pH_start,pH_finish,*fit_sym)
        count+=1
...
def equivalence_point(df,skip=22):
    # calculates the equivalence point by looking for maxium of the derivative (inflection point)
    # skips the first skip=22 points due to initial sharp increase for titration curves
...
def mono_hill_equiv(H,Ka,n,Ainit,Va=Vas[0],equiv=250):
    Kw = 10**(-14) #pK water
    Binit=Cb
    return -(((H**(2 + n) - Ainit*H*Ka**n + H**2*Ka**n - H**n*Kw - Ka**n*Kw)*Va)\
...
def sim_mono_hill_equiv(start_pH, end_pH, *args):
    pH_range = np.linspace(start_pH,end_pH,40)
    H = [10**(-pH) for pH in pH_range]
    params=[args[i] for i in range(0, len(args))]
    plt.xlabel('Added {} M  NaOH ($\mu$L)'.format(Cb),fontsize=18)
    ...
    pKa, n, Ainit=np.round(-np.log10(params[0]),2), np.round(params[1],2),\
                            np.round(params[2],4)
    plt.plot(mono_hill_equiv(10**(-pH_range),*params),pH_range, \
             label="pKa = {}, n = {}, conc (A) = {}".format(pKa,n, Ainit))
    plot_default()
...
def fit_hill_symfit(df,guess,**kwargs):
    ...
    n = Parameter('n',value=guess[1],min=0.4,max=2.2)
    Ainit = Parameter('Ainit',value=guess[2],min=0.004,max=0.008)
    H = Variable('H')
    model = mono_hill_equiv(H,Ka,n,Ainit,Va=Vas[0],equiv=250)
